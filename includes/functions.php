...
function createOrder($conn, $customerData) {
    ...
    try {
        // First, check if the user_id column exists in the orders table
        $columnCheck = mysqli_query($conn, "SHOW COLUMNS FROM orders LIKE 'user_id'");
        $useUserId = ($columnCheck && mysqli_num_rows($columnCheck) > 0);
        
        if ($useUserId) {
            // Use the new table structure with user_id
            $orderStmt = mysqli_prepare($conn, "INSERT INTO orders (user_id, customer_name, customer_email, customer_address, total_amount) VALUES (?, ?, ?, ?, ?)");
            if (!$orderStmt) {
                error_log("Prepare failed for order insert: " . mysqli_error($conn));
                throw new Exception("Prepare failed for order insert: " . mysqli_error($conn));
            }
            
            // Set user_id to NULL for guest orders or use the logged-in user's ID
            $userId = isUserLoggedIn() ? $_SESSION['user_id'] : NULL;
            mysqli_stmt_bind_param($orderStmt, "isssd", $userId, $customerData['name'], $customerData['email'], $customerData['address'], $cart['total']);
        } else {
            // Use the old table structure without user_id (fallback for older databases)
        ...
        if (!$orderResult) {
            error_log("Execute failed for order insert: " . mysqli_stmt_error($orderStmt));
        ...
        if (!$itemStmt) {
            error_log("Prepare failed for order items insert: " . mysqli_error($conn));
        ...
        if (!$stockUpdateStmt) {
            error_log("Prepare failed for stock update: " . mysqli_error($conn));
        ...
        foreach ($cart['items'] as $item) {
            ...
            if (!$itemResult) {
                error_log("Execute failed for order item insert: " . mysqli_stmt_error($itemStmt));
            ...
            if (!$stockResult) {
                error_log("Execute failed for stock update: " . mysqli_stmt_error($stockUpdateStmt));
...
function adminLogin($username, $password) {
    ...
    // Debug logging
    error_log("Admin login attempt - Username: $username, Found user: " . ($admin ? 'Yes' : 'No'));
    ...
    if ($admin && password_verify($password, $admin['password_hash'])) {
        ...
        if ($admin) {
            error_log("Password verification failed for user: " . $admin['username']);
        }
        return false;
    }
}
...
function userLogin($username, $password) {
    ...
    // Debug logging
    error_log("User login attempt - Username/Email: $username, Found user: " . ($user ? 'Yes' : 'No'));
    ...
    if ($user && password_verify($password, $user['password_hash'])) {
        ...
        error_log("User login failed for username/email: $username");
        if ($user) {
...
function getUserOrders($conn, $userId) {
    ...
    if (!is_numeric($userId)) {
        error_log("Invalid user ID in getUserOrders: " . $userId);
        return array();
    }
    ...
    if (!$stmt) {
        error_log("Prepare failed in getUserOrders: " . mysqli_error($conn));
        return array();
    }
    ...
    if (!$executeResult) {
        error_log("Execute failed in getUserOrders: " . mysqli_stmt_error($stmt));
        mysqli_stmt_close($stmt);
        return array();
    }
...
function getOrderItems($conn, $orderId) {
    ...
    if (!is_numeric($orderId)) {
        error_log("Invalid order ID in getOrderItems: " . $orderId);
        return array();
    }
    
    // Use prepared statement to prevent SQL injection
    $stmt = mysqli_prepare($conn, "SELECT oi.*, p.name as product_name, p.image as product_image FROM order_items oi JOIN products p ON oi.product_id = p.id WHERE oi.order_id = ?");
    if (!$stmt) {
        error_log("Prepare failed in getOrderItems: " . mysqli_error($conn));
        return array();
    }
    ...
    if (!$executeResult) {
        error_log("Execute failed in getOrderItems: " . mysqli_stmt_error($stmt));
        mysqli_stmt_close($stmt);
        return array();
    }
